const music = new Audio("arjit_audio/10.mp3");
...
const arjit_songs = [
  {
    id: 1,
    songName: `Naina ashq na ho (Holiday)<br>
        <div class="subtitle">Arjit Singh</div>`,
    poster: "arjit_img/1.jpg",
  },
  {
    id: 2,
    songName: `Khariyat (Chhichhore)<br>
        <div class="subtitle">Pritam, Amitabh B|Arijit Singh</div>`,
  ...
  {
    id: 5,
    songName: `Tera yaar hoon main (SKTKS)<br>
        <div class="subtitle">Arjit Singh</div>`,
    poster: "arjit_img/5.jpg",
  },
  {
    id: 6,
    songName: `Chunar (ABCD 2 ) <br>
        <div class="subtitle">Arijit Singh | Sachin - Jigar</div>`,
    poster: "arjit_img/6.jpg",
  },
  {
    id: 7,
    songName: `Galti se mistake (Jagga Jasoos)<br>
  ...
  {
    id: 9,
    songName: ` Neki ki raah (Traffic)<br>
        <div class="subtitle">Arjit Singh</div>`,
    poster: "arjit_img/9.jpg",
  },
  {
    id: 10,
    songName: `Humdard (Vilain)<br>
        <div class="subtitle">Arjit Singh</div>`,
    poster: "arjit_img/10.jpg",
  },{
    id: 11,
    songName: `Mere Yaara (Soorayavanshi)<br>
        <div class="subtitle">Arjit Singh</div>`,
  ...
  {
    id: 14,
    songName: `Agar Tum Saath Ho (Tamasha)<br>
...
singer_name.innerHTML="Arjit Singh"

let artist_img = document.getElementsByClassName("artist_img");
artist_img[0].innerHTML=`<img src="main_img/arjit_bg.png" alt="" srcset=""> `
console.log(artist_img)

//----------------------------------------------------------

let part_1 = Math.floor(0.5 * arjit_songs.length);
let part_2 = arjit_songs.length - part_1;
// console.log(part_1)

let menue_song = document.getElementsByClassName("menue_song")[0];
// console.log(menue_song)
for (let i = 0; i < part_1; i++) {
  let serial_no = arjit_songs[i].id;
  if (serial_no < 10) {
...
<img src="${arjit_songs[i].poster}" alt="" />
<h6 class="titlelist">
 ${arjit_songs[i].songName}
</h6>
<i class="playListPlay fa fa-solid fa-circle-play" id="${arjit_songs[i].id}"></i>`;
...
// part 2 == popular arjit_songs
let pop_song = document.getElementsByClassName("pop_song");

for (let j = part_1; j < arjit_songs.length; j++) {
  let serial_no = arjit_songs[j].id;
  if (serial_no < 10) {
    serial_no = "0" + serial_no;
  }
  let songItem = document.createElement("li");
...
<img src="${arjit_songs[j].poster}" alt="" />
<i class="playListPlay fa fa-solid fa-circle-play" id="${arjit_songs[j].id}"></i>
</div>
<h6>
${arjit_songs[j].songName}
</h6>`;
...
for (let k = 0; k < arjit_songs.length; k++) {
  // let serial_no = arjit_songs[k].id;
  // if (serial_no < 10) {
  //   serial_no = "0" + serial_no;
  // }
  let card = document.createElement("a");
  card.classList.add("card");
  card.setAttribute("href","#"+ arjit_songs[k].id)
  // console.log(card)
  // card[k].href= "#"+ arjit_songs[k].id
  card.innerHTML = `<img src="${arjit_songs[k].poster}" alt="" />
  <h6 class="sr_title">
  ${arjit_songs[k].songName}
  </h6>`;
...
function control(index, prev) {
  ...
  download_music.setAttribute("download", arjit_songs[index - 1].songName);

  // title and poster --- masterplay
  title.innerHTML = arjit_songs[index - 1].songName;
  poster_master_play.src = arjit_songs[index - 1].poster;
...
const random_music = () => {
  ...
  if (index > songItem.length) {
    ...
    index = Math.floor(Math.random() * arjit_songs.length + 1);
  }
  // console.log(index)
  control(index, prev);
};
